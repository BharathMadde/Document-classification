...
exports.extractText = async (req, res) => {
  ...
  try {
    ...
    let extractMessage = `✅ Text extracted from "${fileName}"! Found ${Object.keys(entities).length} details.`;
    // If extractedText is empty or only whitespace, mark for Human Intervention
    if (!extractedText || !extractedText.trim()) {
      status = "Human Intervention";
      extractMessage = `❌ Unable to extract text from "${fileName}". Sent for human intervention.`;
    }
    ...
    res.json({
      success: true,
      message: status === "Extracted" ? "Extracted successfully" : "Extraction failed, sent for human intervention",
